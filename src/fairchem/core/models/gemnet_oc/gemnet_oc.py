...
class GemNetOC(BaseModel):
    ...
    def forward(self, data):
        ...
        charge_sq_per_atom = charge_per_atom ** 2
        # 3. 拼接charge到原子特征
        x_E_with_charge = torch.cat([x_E, charge_per_atom, charge_sq_per_atom], dim=-1)  # (nAtoms, emb_size_atom+1)
        # 4. 预测原子能量
        ...
        with torch.cuda.amp.autocast(False):
            # E_t = self.out_energy(E_t.float())  # 256 * 1
            if self.direct_forces:
        ...
        # else:
        #     E_t = scatter_det(
        #         x_E, batch, dim=0, dim_size=nMolecules, reduce="mean"
        #     )  # (nMolecules, num_targets)
